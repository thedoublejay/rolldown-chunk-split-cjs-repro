...
const Box9 = styled.div` color: ${tokens.colors.text}; padding: 8px; `;

export function Shared9({ label, children }) {
  const { state, derived } = useShared10(label);
  const { register } = useForm();
  const cls = clsx("shared-9", { active: state });
  return (
    <Paper className={cls}>
      <Stack>
