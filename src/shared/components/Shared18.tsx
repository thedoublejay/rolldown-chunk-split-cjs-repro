...
const Box18 = styled.div` color: ${tokens.colors.text}; padding: 8px; `;

export function Shared18({ label, children }) {
  const { state, derived } = useShared19(label);
  const { register } = useForm();
  const cls = clsx("shared-18", { active: state });
  return (
    <Paper className={cls}>
      <Stack>
