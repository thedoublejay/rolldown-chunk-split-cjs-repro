
import { useState } from "react";
import { Box, Button, Typography } from "@mui/material";
import LegacyContainer10 from "../../../legacy/containers/LegacyContainer10";
import { Shared10 } from "../../../shared/components/Shared10";
import { useLibrary2 } from "../hooks/use-library-2";
export function Library9(props) {
  const [open, setOpen] = useState(false);
  const { keyed, prefix } = useLibrary2(props.label);
  return <Box><Typography>{prefix}: {Object.keys(keyed).length}</Typography><Shared10 label="library9">item</Shared10><LegacyContainer10 title="library-9" /><Button onClick={() => setOpen(!open)}>Toggle</Button></Box>;
}
export default Library9;
