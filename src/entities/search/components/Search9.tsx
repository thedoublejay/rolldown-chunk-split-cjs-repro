
import { useState } from "react";
import { Box, Button, Typography } from "@mui/material";
import LegacyContainer10 from "../../../legacy/containers/LegacyContainer10";
import { Shared10 } from "../../../shared/components/Shared10";
import { useSearch2 } from "../hooks/use-search-2";
export function Search9(props) {
  const [open, setOpen] = useState(false);
  const { keyed, prefix } = useSearch2(props.label);
  return <Box><Typography>{prefix}: {Object.keys(keyed).length}</Typography><Shared10 label="search9">item</Shared10><LegacyContainer10 title="search-9" /><Button onClick={() => setOpen(!open)}>Toggle</Button></Box>;
}
export default Search9;
