
import { useState } from "react";
import { Box, Button, Typography } from "@mui/material";
import LegacyContainer4 from "../../../legacy/containers/LegacyContainer4";
import { Shared4 } from "../../../shared/components/Shared4";
import { useComments4 } from "../hooks/use-comments-4";
export function Comments3(props) {
  const [open, setOpen] = useState(false);
  const { keyed, prefix } = useComments4(props.label);
  return <Box><Typography>{prefix}: {Object.keys(keyed).length}</Typography><Shared4 label="comments3">item</Shared4><LegacyContainer4 title="comments-3" /><Button onClick={() => setOpen(!open)}>Toggle</Button></Box>;
}
export default Comments3;
