
import { useState } from "react";
import { Box, Button, Typography } from "@mui/material";
import LegacyContainer4 from "../../../legacy/containers/LegacyContainer4";
import { Shared4 } from "../../../shared/components/Shared4";
import { useTags4 } from "../hooks/use-tags-4";
export function Tags3(props) {
  const [open, setOpen] = useState(false);
  const { keyed, prefix } = useTags4(props.label);
  return <Box><Typography>{prefix}: {Object.keys(keyed).length}</Typography><Shared4 label="tags3">item</Shared4><LegacyContainer4 title="tags-3" /><Button onClick={() => setOpen(!open)}>Toggle</Button></Box>;
}
export default Tags3;
